...
- 设备本地行为、硬件外设、固件源码、SDK、编译或烧录：转交专用端侧
  Skill。
- 同时需要新增设备能力和云端调用逻辑：分别使用端侧 Skill 和本 Skill，
  不要把整项需求强行归入单一工程。
...
4. Agent 将 Product ID 和 Product Secret 都写入已确认的设备：
   ...
   命令都成功前，不要宣告绑定完成。
5. 使用设备支持的 shell 命令重启：

   ```bash
   adb shell reboot
   ```

   检测到多台设备时使用 `adb -s <serial> shell reboot`。不要改用
   `adb reboot`，也不要把命令退出码当作重启成功。必须观察目标设备先断开、
   再重新上线，并在重连后验证应用配置或实际行为；否则只能报告重启命令已
   发出、尚未验证成功。完成这一步不要求设备端 SDK 或端侧 Skill。
...
1. 先检查当前工作区和 Agent 环境是否已有与目标芯片、开发板及任务匹配的
   端侧开发或烧录 Skill。
2. ARCS Mini 的官方 SDK 位于
   `https://cloud.listenai.com/CSKG836746/arcs-sdk/public/arcs_mini`，仓库内自带
   设备、编码和烧录 Skill。当前工作区没有对应工程或 Skill 时，先向用户说明
   将从该官方仓库获取端侧工程；获得确认后再克隆
   `https://cloud.listenai.com/CSKG836746/arcs-sdk/public/arcs_mini.git`。
3. 进入仓库后读取并遵循与任务匹配的仓库内 Skill，再按已确认范围开展端侧
   工作。不要把仓库内 Skill 脱离源码目录单独使用，也不要把其中的实现细节
   复制进本 Skill。
4. 不是 ARCS Mini，或官方工程内仍找不到匹配 Skill 时，明确告知缺少专用
   端侧能力，并协助用户发现正确的官方 SDK 与 Skill。

没有专用 Skill 时，不要猜测仓库地址、工具链、构建参数、串口或烧录命令。
仅切换 PID/SID 或绑定应用时继续使用上一节，不获取 SDK、不拉取或构建端侧
仓库。
